...
const Navbar = () => {
  ...
  // Build dynamic nav links from real categories
  const navLinks = [
    { label: "Home", to: "/" },
    ...
    ...(categories
      ?.filter((c) => c.name !== "All")
      .slice(0, 6)
      .map((c) => ({ label: c.name, to: `/products?category=${c.name}` })) || []),
    { label: "Cart", to: "/cart" },
  ];
        ...
        {menuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 z-50 h-full w-72 bg-background p-6 shadow-xl overflow-y-auto"
            >
